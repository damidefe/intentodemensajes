...
from AppBlogs.forms import Formulario_blog, UserRegisterForm, UserEditForm, AvatarForm, Formulario_mensaje
from AppBlogs.models import Blog, Avatar , Mensajes
import datetime
...
def form_mensaje(request):
    if (request.method == "POST"):
        formulario = Formulario_mensaje(request.POST)
        if (formulario.is_valid()):
            info = formulario.cleaned_data
            emisor1 = info.get("emisor")
            receptor1 = info.get("receptor")
            mensajef1 = info.get("mensaje")
            mensaje = Mensajes(mensajef = mensajef1, emisor = emisor1, receptor = receptor1 )
            mensaje.save()
            return (render(request, "AppBlogs/inicio.html", {"mensaje": "Mensaje Enviado"}))
        else:
            return (render(request, "AppBlogs/inicio.html", {"mensaje": "Error"}))
